<?php
/**
 * Builds and registers the Catalyst Excerpt Widget.
 *
 * @package Catalyst
 */
...
/**
 * Build the Catayst Excerpt Widget class and function.
 *
 * @package Catalyst
 * @since 1.0
 */
class catalyst_no_fluff_widget extends WP_Widget {

	function catalyst_no_fluff_widget()
	{
		$widget_setup = array( 'classname' => 'cnfw-widget', 'description' => __( 'Displays Excerpts in Widget Areas', 'cnfw' ) );
		$widget_panel = array( 'width' => 505, 'height' => 350, 'id_base' => 'cnfw-widget' );
		$this->WP_Widget( 'cnfw-widget', __( 'Catalyst | No Fluff', 'cnfw' ), $widget_setup, $widget_panel );
	}

	function widget( $args, $options )
	{
		global $wp_query, $catalyst_layout_id, $catalyst_loop_count;
		...
		//echo $before_widget;
		echo '<div class="bl-content">';

		if( $options['content-display'] == "latest_post" )
		{
			$featured_content = new WP_Query( array( 'ignore_sticky_posts' => 1, 'post_type' => 'post', 'showposts' => $options['num-posts'], 'offset' => $options['offset'] ) );
		}
		...
		//echo '<div class="catalyst-cnfw-widget-inner">';
		...
		echo '</div>';
		//echo '<div class="cnfw-widget-clear-both" style="clear:both;"></div>';
	...
	function form( $options )
	{
		...
		$image_url = get_template_directory_uri() . '/lib/css/images/catalyst-admin-bg-box-bg.png';
	?>
		...
		<div style="width:250px; float:left;">
			...
			<div style="background:#F1F1F1 url( <?php echo $image_url ?> ) repeat-x; border:1px solid #E3E3E3; margin-bottom:10px; padding:10px 10px 0;">
				...
				<p>
					<?php
					$excerpt_limit = empty( $options['excerpt-limit'] ) ? '' : $options['excerpt-limit'];
					$char_limit = empty( $options['char-limit'] ) ? '' : $options['char-limit'];
					?>
...
add_action( 'widgets_init', create_function( '', "register_widget( 'catalyst_no_fluff_widget' );" ) );

//end lib/widgets/catalyst-cnfw-widget.php